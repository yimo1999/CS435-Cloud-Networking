...
void forwardingTable(){
	...
	memset(forwardTable, 0, sizeof(*forwardTableTemp));
...
void* checkTimeout(void* unusedParam){
	...
	while(1){
		...
		for(int i = 0; i < 256; i++){
			...
			if(neighbors[i] == 1 && currTime.tv_sec - globalLastHeartbeat[i].tv_sec > 4){
					ifTimeout = 1;
    ...
    const char* p = (const char*)(char*)recvBuf;
		if(!strncmp(msg->type, "send", 4))
		{
			...
			if(msg->dest == globalMyID){
				...
				// char* text_pointer = msg->content;
				// string text = to_string(text_pointer);
		...
		else if(!strncmp(msg->type, "cost", 4))
		{
			...
			if(updateinfo[u->nodeID] == 256){
				updateinfo[u->nodeID] = 0;
				changeFlag = 1;
				shortestPath();
			}
